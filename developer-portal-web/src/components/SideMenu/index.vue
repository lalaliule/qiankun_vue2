    ...
    <el-aside :style="{ width: `${!isCollapse ? '250px' : '65px'}` }">
      ...
      >
        <template v-for="item in menuJson">
          ...
          <template v-else>
            <el-menu-item :index="isQianKun?'/qk_s2'+item.normalUrl:item.normalUrl" :key="item.normalUrl">
  ...
  template: `
    <el-submenu :index="menuInfo.normalUrl" v-bind="$props" v-on="$listeners">
     ...
     <template v-for="v in menuInfo.children">
        ...
        <template v-else>
         <el-menu-item :index="${window.__POWERED_BY_QIANKUN__}?'/qk_s2'+v.normalUrl:v.normalUrl">
